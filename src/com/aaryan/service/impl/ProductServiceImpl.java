...
import com.aaryan.beans.DemandBean;
import com.aaryan.beans.ProductBean;
import com.aaryan.service.ProductService;
import com.aaryan.utility.DBUtil;
import com.aaryan.utility.IDUtil;
		...
		try {
			...
			if ((k > 0) && (prevQuantity < updatedProduct.getProdQuantity())) {
				...
				for (DemandBean demand : demandList) {
					...
					if (flag)
						status += " And notified customers who were waiting for this product!";
				}
			} else if (k > 0)
				status = "Product Updated Successfully!";
			else
